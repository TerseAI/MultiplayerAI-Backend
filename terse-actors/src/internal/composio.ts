...
import type { AgentToolExecution } from "./types.js"
...
export class ComposioTools {
    ...
    async definitions(sessionId: string, enabledTools: string[]): Promise<ComposioToolDefinition[]> {
    ...
    async execute(sessionId: string, tool: string, arguments_: Record<string, unknown>): Promise<AgentToolExecution> {
        const session = await this.composio.use(sessionId)
        ...
        return {
            tool,
            data: z.record(z.string(), z.json()).parse(result.data),
            logId: result.logId
        }
    }
}
