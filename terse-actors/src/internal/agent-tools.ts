...
import { ComposioTools } from "./composio.js"
...
export async function executeTool(runtime: AgentRuntime, tool: string, arguments_: Record<string, unknown>) {
...
export async function modelTools(runtime: AgentRuntime, configuration?: AgentToolConfig): Promise<AgentTool[]> {
    ...
    const definitions = await composioTools(runtime).definitions(config.composioSessionId, config.enabledTools)
