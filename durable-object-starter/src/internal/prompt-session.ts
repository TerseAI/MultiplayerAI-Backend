...
export async function acquirePromptLock(
	...
	await dependencies.store.schedulePromptLockExpiration(lock.expiresAt);
...
export async function updatePromptDraft(
	...
	if (
		attachments &&
		(attachments.length > MAX_PROMPT_IMAGES || !attachments.every(validImageAttachment))
	) {
	...
	if (!lock || lock.user.connectionId !== connectionId || lock.leaseId !== leaseId) {
		throw new Error(
			lock ? `${lock.user.name} is editing the prompt` : "Reserve the composer before editing",
		);
	}
...
export async function currentPromptDraft(store: AgentStore): Promise<ComposerDraft> {
	...
	return stored
		? {
				...stored,
				attachments: Array.isArray(stored.attachments)
					? stored.attachments.filter(validImageAttachment)
					: [],
			}
		: emptyPromptDraft();
}
...
export async function currentPromptLock(
	dependencies: PromptSessionDependencies,
): Promise<ComposerLock | null> {
...
export async function handlePromptSessionAlarm(
	dependencies: PromptSessionDependencies,
): Promise<void> {
	const lock = await dependencies.store.promptLock();
	if (!lock) return;
	...
	await clearPromptLock(dependencies, false);
}
...
export function assertOwnsPromptLock(
	...
	if (
		lock &&
		lock.user.connectionId === user.connectionId &&
		lock.leaseId === leaseId
	) return;

	throw new Error(
		lock ? `${lock.user.name} is editing the prompt` : "Reserve the composer before sending",
	);
}
...
async function clearPromptLock(
	dependencies: PromptSessionDependencies,
	clearAlarm = true,
): Promise<void> {
	await dependencies.store.deletePromptLock();
	if (clearAlarm) await dependencies.store.clearPromptLockExpiration();
